...
def play_games(
    player1: Player, player2: Player, game_module, num_games: int
) -> Dict[str, int]:
...
def create_player(
    ...
    if player_type == "random":
        ...
        return NetworkMCTSPlayer(
            ...
            name=name or f"MCTS({Path(model_path).stem})",
        )
        ...
        raise ValueError(
            f"Unknown player type: {player_type}. Available: random, greedy, human, network-mcts, network-direct"
        )
...
def main():
    ...
    parser.add_argument(
        "--player1",
        required=True,
        choices=["random", "greedy", "human", "network-mcts", "network-direct"],
    ...
    parser.add_argument(
        "--player1-model", help="Model checkpoint path for player 1 (if network player)"
    )
    parser.add_argument(
        "--player2-model", help="Model checkpoint path for player 2 (if network player)"
    )
    # Game configuration
    parser.add_argument(
    ...
    try:
        ...
        print(
            f"  {player1.name}: {results['player1_wins']} wins ({results['player1_win_rate']:.1%})"
        )
        print(
            f"  {player2.name}: {results['player2_wins']} wins ({results['player2_win_rate']:.1%})"
        )
