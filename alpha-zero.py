...
def train_network(
    ...
    target_values = torch.tensor([sample['value'] for sample in training_samples], dtype=torch.float32).unsqueeze(1)
    ...
    for epoch in range(config.epochs):
        ...
        batch_pbar = tqdm(dataloader, desc=f"Epoch {epoch+1}/{config.epochs}", leave=False)
        ...
        for batch_states, batch_target_policies, batch_target_values in batch_pbar:
            ...
            policy_loss = -torch.sum(batch_target_policies * log_pred_policies, dim=1).mean()
    ...
    final_stats = epoch_losses[-1] if epoch_losses else {"avg_loss": 0, "avg_policy_loss": 0, "avg_value_loss": 0}
    return final_stats


def save_checkpoint(model: network.Model, optimizer, iteration: int, config: TrainingConfig, training_stats: list) -> None:
    ...
    if training_stats and len(training_stats) > 0:
        ...
        if not best_path.exists():
            ...
            if best_stats and latest_loss < min(stat['avg_loss'] for stat in best_stats[-10:]):
                torch.save(checkpoint, best_path)
...
def training_loop(config: TrainingConfig) -> None:
    ...
    for iteration in tqdm(range(1, config.iterations + 1), desc="Training iterations"):
        ...
        # Training phase
        tqdm.write(f"🏋️  Training network...")
        ...
        # Display progress
        tqdm.write(f"📈 Iteration {iteration} complete - "
